@extends('layouts.support')


@section('title', 'About us')

@section('content')

  <!-- Start Page Banner Area -->
  <div class="page-banner-area">
      <div class="container">
          <div class="row">
              <div class="col-md-12">
                  <div class="page-title">
                      <h2>About-us Page</h2>
                      <span class="sub-title"><a href="#">Home</a> / About-us Page</span>
                  </div>
              </div>
          </div>
      </div>
  </div>
  <!-- End Pages Banner Area -->
  <!-- Start Our Community -->
  <div class="our-community-area themeix-ptb">
      <div class="container">
          <div class="row">
              <div class="col-sm-4">
                  <div class="single-community">
                      <div class="icons"><i class="fa fa-plug"></i></div>
                      <h3>Successful Pivot</h3>
                      <p>The founders of Altpocket were previously working on a e-Learning platform, however, in May, 2017, they decided to move onto Altpocket due to the demand of its services.</p>
                  </div>
              </div>
              <div class="col-sm-4">
                  <div class="single-community">
                      <div class="icons"><i class="fa fa-random"></i></div>
                      <h3>Constantly Evolving</h3>
                      <p>Altpocket was initially a side project that was released in May, 2017, as a portfolio, however since then it has evolved to be more than just a portfolio.</p>
                  </div>
              </div>
              <div class="col-sm-4">
                  <div class="single-community">
                      <div class="icons"><i class="fa fa-rocket"></i></div>
                      <h3>To The Moon</h3>
                      <p>Altpocket is aiming for the moon, our goal is to become more than just a portfolio, we want to be the only platform you need for managing cryptocurrency investments.</p>
                  </div>
              </div>
          </div>
      </div>
  </div>
  <!-- End Our Community -->
  <!-- Start Our History Area -->
  <div class="our-history-area themeix-ptb info-bg">
      <div class="container">
          <div class="row">
              <div class="col-md-offset-3 col-md-6">
                  <div class="themeix-section-title text-center">
                      <h2>Our History</h2>
                      <p>From a less successful company to a company that is used by thousands.</p>
                  </div>
              </div>
          </div>
          <div class="row">
              <div class="timeline timeline-line-dotted">
                  <span class="timeline-label">
                      <span class="label label-primary">05.09.2016</span>
                  </span>
                  <div class="timeline-item">
                      <div class="timeline-point timeline-point-success">
                          <i class="fa fa-angle-double-down"></i>
                      </div>
                      <div class="timeline-event">
                          <div class="timeline-heading">
                              <h4>The Story Began</h4>
                          </div>
                          <div class="timeline-body">
                              <p>In early September, 2016 we started development of our first project, company and platform - Learnimore, which was going to become a E-Learning platform later.</p>
                          </div>
                      </div>
                  </div>
                  <span class="timeline-label">
                      <span class="label label-primary">10.03.2017</span>
                  </span>
                  <div class="timeline-item">
                      <div class="timeline-point timeline-point-success">
                          <i class="fa fa-angle-double-down"></i>
                      </div>
                      <div class="timeline-event">
                          <div class="timeline-heading">
                              <h4>Learnimore Officially Launched</h4>
                          </div>
                          <div class="timeline-body">
                              <p>In early March, 2017, Learnimore's platform was complete and launched to the public.</p>
                          </div>
                      </div>
                  </div>
                  <span class="timeline-label">
                      <span class="label label-primary">13.05.2017</span>
                  </span>
                  <div class="timeline-item">
                      <div class="timeline-point timeline-point-success">
                          <i class="fa fa-angle-double-down"></i>
                      </div>
                      <div class="timeline-event">
                          <div class="timeline-heading">
                              <h4>Learnimore Shutdown, Altpocket Began</h4>
                          </div>
                          <div class="timeline-body">
                              <p>Learnimore was not as much of a success as we hoped, on this day we not only launched Altpocket which had been a sideproject of the developer, but we decided that this is what we want to do.</p>
                          </div>
                      </div>
                  </div>
                  <span class="timeline-label">
                      <span class="label label-primary">Today</span>
                  </span>
                  <div class="timeline-item">
                      <div class="timeline-point timeline-point-success">
                          <i class="fa fa-angle-double-down"></i>
                      </div>
                      <div class="timeline-event">
                          <div class="timeline-heading">
                              <h4>Altpocket today</h4>
                          </div>
                          <div class="timeline-body">
                              <p>Barely 2 months has gone and we have over 5000 registered users and we track over 120 thousand investments. The future is bright for Altpocket.</p>
                          </div>
                      </div>
                  </div>
             </div>
        </div>
     </div>
    </div>
  <!-- End Our History Area -->
  <!-- Start About-us Bg -->
  <div class="about-us-bg" style="background:none;">
      <div class="container">
          <div class="row">
              <div class="col-md-12">
                  <div class="about-us-info">
                      <h2>Altpocket is here to stay, register today and track your cryptocurrency investments among thousands of users.</h2>
                      <a class="themeix-btn primary-bg" href="/login">Register Today</a>
                  </div>
              </div>
          </div>
      </div>
  </div>
  <!-- End About-us Bg -->
  <!-- Our Team Member -->
  <section class="team-member themeix-ptb">
      <div class="container">
          <div class="row">
              <div class="col-md-offset-3 col-md-6">
                  <div class="themeix-section-title text-center">
                      <h2>Our Team Members</h2>
                  </div>
              </div>
          </div>
          <div class="team-member-wrapper owl-carousel">
              <div class="single-team wow zoomIn animated">
                  <div class="team-member-img">
                      <a href="#"><img src="https://altpocket.io/uploads/avatars/1/NlrnqID.png" alt="member"></a>
                      <div class="social-profile">
                          <ul>
                              <li><a href="https://twitter.com/altpocket"><i class="fa fa-twitter"></i></a></li>
                              <li><a href="https://www.linkedin.com/in/edwin-flataunet-32b7a612a/"><i class="fa fa-linkedin"></i></a></li>
                          </ul>
                      </div>
                  </div>
                  <h5><a href="#">Mr. Edwin Flataunet</a></h5>
                  <h6>Developer, Founder</h6>
              </div>
              <div class="single-team wow zoomIn animated">
                  <div class="team-member-img">
                      <a href="#"><img src="https://altpocket.io/uploads/avatars/359/jonathan-altpocket-test.png" alt=""></a>
                      <div class="social-profile">
                          <ul>
                              <li><a href="https://twitter.com/altpocket"><i class="fa fa-twitter"></i></a></li>
                              <li><a href="https://www.linkedin.com/in/jonathan-svensson-416336120/"><i class="fa fa-linkedin"></i></a></li>
                          </ul>
                      </div>
                  </div>
                  <h5><a href="#">Mr. Jonathan Svensson</a></h5>
                  <h6>Marketing Manager, Co-Founder</h6>
              </div>
              <div class="single-team wow zoomIn animated">
                  <div class="team-member-img">
                      <a href="#"><img src="https://media.licdn.com/mpr/mpr/shrinknp_400_400/AAEAAQAAAAAAAAqwAAAAJDk3MmI3ZmExLTZmOTEtNGQ4MS05MDU0LTcxZmUzMTNmZGM4OQ.jpg" alt="member"></a>
                      <div class="social-profile">
                          <ul>
                              <li><a href="https://twitter.com/eivindvr"><i class="fa fa-twitter"></i></a></li>
                              <li><a href="https://www.linkedin.com/in/eivindvr/"><i class="fa fa-linkedin"></i></a></li>
                          </ul>
                      </div>
                  </div>
                  <h5><a href="#">Mr. Eivind Vogel-Rödin</a></h5>
                  <h6>Advisor &amp; Mentor</h6>
              </div>
              <div class="single-team wow zoomIn animated">
                  <div class="team-member-img">
                      <a href="#"><img src="https://altpocket.io/uploads/avatars/20/IMG_4020.JPG" alt="member"></a>
                      <div class="social-profile">
                          <ul>
                              <li><a href="https://twitter.com/ErikErvik"><i class="fa fa-twitter"></i></a></li>
                              <li><a href="https://www.linkedin.com/in/erikervikbangsund/"><i class="fa fa-linkedin"></i></a></li>
                          </ul>
                      </div>
                  </div>
                  <h5><a href="#">Mr. Erik Ervik</a></h5>
                  <h6>Administrator</h6>
              </div>
              <div class="single-team wow zoomIn animated">
                  <div class="team-member-img">
                      <a href="#"> <img src="https://altpocket.io/uploads/avatars/462/IMG_0498_Cropped.jpg" alt="member"></a>
                  </div>
                  <h5><a href="#">Mr. Lewis</a></h5>
                  <h6>Administrator</h6>
              </div>
          </div>
      </div>
  </section>
  <!-- End Our Team Member -->

@endsection
